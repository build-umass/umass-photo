...
export default function EventManagementTab() {
  ...
  const refreshData = async () => {
    ...
    const eventRecord = eventList.map((ev: Tables<"event">) => [String(ev.id), ev])
  ...
  const setEventById = (id: string, ev: Tables<"event">) => {
    const newEventData: Record<string, Tables<"event">> = { ...eventData } as Record<string, Tables<"event">>
    newEventData[id] = ev;
    setEventData(newEventData);
  }

  const saveChanges = async () => {
    if (eventData === null) throw new Error("Called saveChanges with eventData==null!")
