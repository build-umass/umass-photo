...
export async function GET(request: NextRequest) {
  ...
  const queryTags = searchParams.get("querytags")
    ? JSON.parse(searchParams.get("querytags")!)
    : [];
  const queryAuthor =
    filteringAuthors && searchParams.get("queryauthor")
  ...
  const { data: filteredPhotos, error } = await client.rpc(
    "filter_photos",
    {
  ...
  const authorIds = [
    ...new Set(filteredPhotos?.map((p: Tables<"photo">) => p.authorid) || []),
  ];
  ...
  const photosWithUrls =
    filteredPhotos?.map((photo: Tables<"photo">) => {
