...
export async function POST(request: NextRequest) {
  ...
  return new Response(JSON.stringify({
    ...
    status: 200,
    headers: [
      ["Set-Cookie", `access-token=${access_token}; SameSite=strict; HttpOnly; Secure; Path=/api`],
      ["Set-Cookie", `refresh-token=${refresh_token}; SameSite=strict; HttpOnly; Secure; Path=/api/refresh`]
    ]
  });
}
