...
CREATE OR REPLACE FUNCTION "public"."filter_photos"("filtering_tags" boolean, "filtering_authors" boolean, "filtering_date" boolean, "querytags" json, "queryauthor" "uuid", "querystart" timestamp with time zone, "queryend" timestamp with time zone) RETURNS SETOF "public"."photo"
    LANGUAGE "sql"
    AS $$
SELECT *
FROM photo p
...
ALTER FUNCTION "public"."filter_photos"("filtering_tags" boolean, "filtering_authors" boolean, "filtering_date" boolean, "querytags" json, "queryauthor" "uuid", "querystart" timestamp with time zone, "queryend" timestamp with time zone) OWNER TO "postgres";
...
CREATE POLICY "Allow admins to manage users" ON "public"."photoclubuser" WITH CHECK (( SELECT "photoclubrole"."is_admin"
...
CREATE POLICY "Allow everyone to insert photos that they are authors of" ON "public"."photo" FOR INSERT WITH CHECK ((( SELECT "auth"."uid"() AS "uid") = "authorid"));



CREATE POLICY "Allow everyone to insert their own profiles if they set safe ro" ON "public"."photoclubuser" FOR INSERT WITH CHECK (((( SELECT "auth"."uid"() AS "uid") = "id") AND (( SELECT "photoclubrole"."is_admin"
   FROM "public"."photoclubrole"
  WHERE (("photoclubrole"."roleid")::"text" = ("photoclubuser"."role")::"text")) = false)));



CREATE POLICY "Allow everyone to select blogs" ON "public"."blog" FOR SELECT TO "authenticated" USING (true);



CREATE POLICY "Allow everyone to select events" ON "public"."event" FOR SELECT TO "authenticated" USING (true);



CREATE POLICY "Allow everyone to select photos" ON "public"."photo" FOR SELECT TO "authenticated" USING (true);



CREATE POLICY "Allow everyone to select phototags" ON "public"."phototag" FOR SELECT TO "authenticated" USING (true);



CREATE POLICY "Allow everyone to select roles" ON "public"."photoclubrole" FOR SELECT TO "authenticated" USING (true);



CREATE POLICY "Allow everyone to select tags" ON "public"."tag" FOR SELECT TO "authenticated" USING (true);



CREATE POLICY "Allow everyone to select users" ON "public"."photoclubuser" FOR SELECT TO "authenticated" USING (true);



CREATE POLICY "Only allow eboard to insert events" ON "public"."event" TO "authenticated" USING (((( SELECT "photoclubuser"."role"
   FROM "public"."photoclubuser"
  WHERE (( SELECT "auth"."uid"() AS "uid") = "photoclubuser"."id")))::"text" = 'eboard'::"text"));
...
GRANT ALL ON FUNCTION "public"."filter_photos"("filtering_tags" boolean, "filtering_authors" boolean, "filtering_date" boolean, "querytags" json, "queryauthor" "uuid", "querystart" timestamp with time zone, "queryend" timestamp with time zone) TO "anon";
GRANT ALL ON FUNCTION "public"."filter_photos"("filtering_tags" boolean, "filtering_authors" boolean, "filtering_date" boolean, "querytags" json, "queryauthor" "uuid", "querystart" timestamp with time zone, "queryend" timestamp with time zone) TO "authenticated";
GRANT ALL ON FUNCTION "public"."filter_photos"("filtering_tags" boolean, "filtering_authors" boolean, "filtering_date" boolean, "querytags" json, "queryauthor" "uuid", "querystart" timestamp with time zone, "queryend" timestamp with time zone) TO "service_role";
